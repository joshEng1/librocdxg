...
size_t CmdUtil::BuildDispatch(
  ...
  const auto workgroup_count = [](uint32_t grid, uint16_t workgroup) -> uint32_t {
    if (workgroup == 0)
      return 0;

    return (grid + workgroup - 1) / workgroup;
  };
  ...
  if (AMD_HSA_BITS_GET(pInfo->pKernelObject->kernel_code_properties,
    ...
    dispatch.compute_user_data_regs.compute_user_data[sgpr_no++] =
      workgroup_count(pInfo->pPacket->grid_size_x, pInfo->pPacket->workgroup_size_x);
  }
  if (AMD_HSA_BITS_GET(pInfo->pKernelObject->kernel_code_properties,
		       AMD_KERNEL_CODE_PROPERTIES_ENABLE_SGPR_GRID_WORKGROUP_COUNT_Y)) {
    dispatch.compute_user_data_regs.compute_user_data[sgpr_no++] =
      workgroup_count(pInfo->pPacket->grid_size_y, pInfo->pPacket->workgroup_size_y);
  }
  if (AMD_HSA_BITS_GET(pInfo->pKernelObject->kernel_code_properties,
		       AMD_KERNEL_CODE_PROPERTIES_ENABLE_SGPR_GRID_WORKGROUP_COUNT_Z)) {
    dispatch.compute_user_data_regs.compute_user_data[sgpr_no++] =
      workgroup_count(pInfo->pPacket->grid_size_z, pInfo->pPacket->workgroup_size_z);
  }
